...
export function AppShell({ children }: PropsWithChildren) {
  ...
  const [preferredGame, setPreferredGame] = useState<HubGame>("osu");
  ...
  useEffect(() => {
    ...
    try {
      if (routeGame) {
        ...
        setPreferredGame(localStorage.getItem("aimmod-hub-game") === "kovaaks" ? "kovaaks" : "osu");
      }
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (((e.ctrlKey || e.metaKey) && e.key === "k") || (e.key === "/" && !editing)) {
        e.preventDefault();
        const input = searchRef.current ?? document.getElementById("beatmap-search") as HTMLInputElement | null;
        input?.focus();
        input?.select();
      }
  ...
  return (
    <div className={`hub-shell${finder ? " hub-shell-finder" : ""}`} data-game={choosing ? "choose" : game}>
      <a className="hub-skip" href="#main-content">Skip to content</a>
      <header className="hub-header">
        <Link to="/" className="hub-brand" aria-label="AimMod Hub home">
        ...
        {!choosing && <select className="hub-game-selector" aria-label="Game" value={game} onChange={e => { const next = e.target.value as HubGame; setPreferredGame(next); navigate(next === "osu" ? "/osu" : "/kovaaks"); }}>
          <option value="osu">osu!</option><option value="kovaaks">KovaaK's</option>
        </select>}
        {choosing ? <nav className="hub-home-nav" aria-label="Games"><Link to="/osu">osu!</Link><Link to="/kovaaks">KovaaK’s</Link><Link to="/app">Downloads</Link></nav> : finder ? <nav className="hub-browse-nav" aria-label="Explore osu!"><Link to="/osu">osu! hub</Link><Link to="/osu/skins">Skins</Link><Link to="/app/osu">Get AimMod</Link></nav> : <div className="hub-header-search"><HeaderSearch key={game} ref={searchRef} game={game} /></div>}
        ...
        {!choosing && <details className="hub-mobile-nav" ref={menuRef}>
