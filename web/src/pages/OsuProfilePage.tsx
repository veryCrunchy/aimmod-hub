...
export function OsuProfilePage() {
  ...
  if (error) {
    return <PageStack><PageSection><EmptyState title="osu! profile not found" body={error} /></PageSection></PageStack>;
  }
  if (!profile) {
    return <PageStack><PageSection><Skeleton className="h-28" /></PageSection><PageSection><Skeleton className="h-72" /></PageSection></PageStack>;
  }
    ...
    <PageStack>
      ...
      <PageSection className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-5 max-[560px]:grid-cols-1">
        ...
        <div className="min-w-0">
          <SectionHeader eyebrow={`@${profile.hubHandle}`} title={profile.osuUsername} body={profile.hubDisplayName} />
